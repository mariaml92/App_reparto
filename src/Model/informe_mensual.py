...
# En este caso, primer y último día del mes anterior al mes en curso.
fecha_actual = dt.date.today()
final_mensual = fecha_actual - dt.timedelta(days = fecha_actual.day)
inicio_mensual = final_mensual - dt.timedelta(days = final_mensual.day - 1)
...
try:
    cur.execute("""INSERT INTO informe_metricas (tipo, fecha_informe, r2, rmse, mape)
                        VALUES %s, %s, %s, %s, %s;""",
                ("mensual", fecha_actual, r2, rmse, mape))
    conn.commit()
except Exception as error:
    print(error)
    conn.rollback()
