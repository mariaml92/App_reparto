...
def pedido_recibido(name):
    ...
    st.write("Pedidos Recibidos por la tienda")
    ...
    df_pedidos_recibidos = df_pedidos_recibidos.rename(columns={'id_pedido': 'ID Pedido', 'id_cliente':'ID Cliente','id_comercio': 'ID Comercio', 'datetime_pedido': 'Fecha','status': 'Estado','tamaño':'Tamaño','direccion':'Direccion'})
    
    # Select only the columns "id_pedido", "status", and "datetime_pedido"
    df_pedidos_recibidos = df_pedidos_recibidos[['ID Pedido', 'ID Cliente', 'ID Comercio', 'Fecha', 'Estado', 'Tamaño', 'Direccion']]
    ...
    if st.button('Cambiar el estado', disabled=not criteria_selected):
        with st.spinner('Cambiando el estado......'):
            ...
            # Establish a connection to the database
                ...
                filtered_df = df_pedidos_recibidos[df_pedidos_recibidos["ID Pedido"] == int(user_input)]
                ...
                st.write(id_repartidor_closest)
                ...
                cursor.execute("UPDATE pedidos SET status = %s , id_repartidor = %s  WHERE id_pedido = %s", (selected_option,id_repartidor_closest, int(user_input)))
